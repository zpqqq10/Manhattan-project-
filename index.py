...
class Node():
    ...
    def index_of_insert(self, key):
        ...
        raise Exception(
            'SYSTEM ERROR: The storage is full. The key has no position to place')
...
class index_manager():
    ...
    def search(self, name, value, type, length):
        ...
        while True:
            ...
            cur_offset = 3
            for i in range(num):
                children.append(struct.unpack(
                    '=hh', cur_block[cur_offset:cur_offset+4]))
                cur_offset += 4
                key, = struct.unpack(
                    '='+type, cur_block[cur_offset:cur_offset+length])
                keys.append(key)
                cur_offset += length
                if type[-1:] == 's':
        ...
        while True:
            ...
            if isleaf == False:
                ...
                if operate == 0 or operate == 1:
                    ...
                    if cur_bid < 0:
                        return result
                    cur_block = self.buffer_manager.read_block(
                        name, 1, cur_bid)
                    isleaf, num = struct.unpack('=?h', cur_block[:3])
                    while isleaf:
                        cur_offset = 3
        ...
        while len(stack) != 0:
            ...
            if not node.is_leaf():
                ...
                self.buffer_manager.write(
                    index_name, 1, bid, cur_offset, content, 3)
                cur_offset += 3
                for i in range(num):
                    content = struct.pack('=hh', node.children[i].bid, 0)
                    self.buffer_manager.write(
                        index_name, 1, bid, cur_offset, content, 4)
                    cur_offset += 4
                    # data read from file is encoded
                    content = struct.pack('='+type, node.keys[i])
                    self.buffer_manager.write(
                        index_name, 1, bid, cur_offset, content, length)
                    cur_offset += length
            ...
            # a leaf
                ...
                for i in range(num):
                    content = struct.pack(
                        '=hh', node.children[i][0], node.children[i][1])
                    self.buffer_manager.write(
                        index_name, 1, bid, cur_offset, content, 4)
                    cur_offset += 4
                    # data read from file is encoded
    ...
    def drop_index_file(self, index_name):
        os.chdir(sys.path[0])
        indpath = './index/'+index_name+'.ind'
        if pathlib.Path(indpath).exists():
            os.remove(indpath)
